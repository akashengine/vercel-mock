...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { assistantId, limit } = req.query as { assistantId?: string; limit?: string };
  try {
    const params = new URLSearchParams();
    if (assistantId) params.set('assistantId', assistantId);
    if (limit) params.set('limit', limit);
    const r = await fetch(`${VAPI_BASE_URL}/call?${params.toString()}`, { headers: { ...authHeader(token) } });
    const data = await r.json();
    if (!r.ok) return res.status(r.status).json(data);
    res.json(data);
  } catch (e: any) {
    res.status(500).json({ error: e?.message || 'List calls failed' });
  }
}
